...
public class SimpleRouteBuilder extends RouteBuilder {
	...
	public void configure() throws Exception {
		...
		from("file:./inboxREST?noop=true").setHeader(Exchange.HTTP_METHOD, simple("GET"))
				.to("http://localhost:8080/employee?id=5").process(new MyProcessor());

		// route for REST POST Call
		from("file:./inboxPOST?noop=true").process(new CreateEmployeeProcessor()).marshal(jsonDataFormat)
